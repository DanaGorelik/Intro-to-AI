...
def find_best_pruning_m(train_dataset: np.array, m_choices, num_folds=5):
    ...
    for i, m in enumerate(m_choices):
        ...
        # ====== YOUR CODE: ======
        kf = sklearn.model_selection.KFold(n_splits=num_folds, shuffle=True, random_state=209398007)
        acc = []
        for train, validation in create_train_validation_split(train_dataset, kf, validation_ratio=0.2):
            x_train, y_train, x_test, y_test = get_dataset_split(train, validation, target_attribute)
...
def cross_validation_experiment(plot_graph=True):
    ...
    best_m, accuracies = find_best_pruning_m(train_dataset, m_choices, num_folds=5)
