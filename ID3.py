...
class ID3:
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        left_weight = len(left_labels) / float(total_labels)
        right_weight = len(right_labels) / float(total_labels)
    ...
    def find_best_split(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        for feat_index, feature in enumerate(self.label_names[1:]):
            vees = sorted([row[feat_index] for row in rows])
            thresholds = [0.5*(vees[i] + vees[i+1]) for i in range(len(vees) - 1)]
            for threshold in thresholds:
                question = Question(feature, feat_index, threshold)
    ...
    def build_tree(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        if np.all(labels == labels[0]) or len(rows) <= self.min_for_pruning:
